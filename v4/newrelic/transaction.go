...
func (txn *Transaction) AddAttribute(key string, val interface{}) {
	root := txn.getRootSpan()
	if root == nil {
		return
	}
	root.Span.SetAttributes(label.Any(key, val))
}
...
func (txn *Transaction) InsertDistributedTraceHeaders(hdrs http.Header) {
	...
	otel.InjectHTTP(currentSpan.ctx, txn.app.propagators, hdrs)
}
...
func (txn *Transaction) AcceptDistributedTraceHeaders(t TransportType, hdrs http.Header) {
	...
	remoteCtx := otel.ExtractHTTP(context.Background(), txn.app.propagators, hdrs)
