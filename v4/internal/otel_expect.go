...
type OpenTelemetryExpect struct {
	Spans *tracetest.StandardSpanRecorder
}

func spansMatch(want WantSpan, span *tracetest.Span, exactAttrs bool) error {
	name := span.Name()
	if want.Name != "" {
		if name != want.Name {
...
func (e *OpenTelemetryExpect) spans() []*tracetest.Span {
